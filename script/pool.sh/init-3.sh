...
forge-script ./script/pool/enlist-token.s.sol:Run -f $FORK_URL -s 'run(uint,string memory,uint)' $POOL_IDX USDT 1 --verify
forge-script ./script/pool/enwrap-token.s.sol:Run -f $FORK_URL -s 'run(uint,string memory,uint)' $POOL_IDX APOW 0 --verify
forge-script ./script/pool/enwrap-token.s.sol:Run -f $FORK_URL -s 'run(uint,string memory,uint)' $POOL_IDX USDT 1 --verify
...
# forge-script ./script/pool/cap-supply.s.sol:Run -f $FORK_URL -s 'run(uint,string memory,uint)' $POOL_IDX USDT $MAX
...
forge-script ./script/position/enroll.s.sol:Run -f $FORK_URL -s 'run(uint,string memory)' $POOL_IDX USDT
