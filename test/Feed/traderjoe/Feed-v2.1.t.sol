...
import {MockFeed_V2, MockPrice} from "./MockFeed-v2.1.sol";
import {Test} from "forge-std/Test.sol";

contract BaseTest is Test {
    MockFeed_V2 internal immutable _feed;
    MockPrice[] internal _prices;

    constructor() {
        _prices.push(MockPrice(2e18, 1e18));
        _feed = new MockFeed_V2(_prices);
    }
}
...
contract FeedTest_InsufficientLiquidity is BaseTest {
    function test_getQuotes_max() public {
        uint256 amount = type(uint128).max + uint256(1) - 1e18;
        vm.expectRevert(
