...
contract PoolLiquidate_Only is PoolLiquidate {
    function setUp() public override {
        ...
        ///
        vm.startPrank(dada);
        USDC.approve(address(pool), bUSDC.totalOf(caca));
        AVAX.approve(address(pool), sAVAX.totalOf(caca));
        vm.stopPrank();
    }

    function test_liquid_only() public {
        vm.startPrank(dada);
        pool.liquidate(caca, 0);
        vm.stopPrank();
    }
}
...
contract PoolLiquidate_After is PoolLiquidate {
    ...
    function test_balance_of_vault() public view {
        assertEq(AVAX.balanceOf(address(vAVAX)), 100 * AVAX_ONE);
        assertEq(USDC.balanceOf(address(vUSDC)), 100 * USDC_ONE);
    }

    function test_balance_of_pool() public view {
        assertEq(vAVAX.balanceOf(address(pool)), 100e9 * AVAX_ONE);
        assertEq(vUSDC.balanceOf(address(pool)), 100e9 * USDC_ONE);
    }
    ...
    function test_balance_of_dada() public view {
        assertEq(AVAX.balanceOf(dada), 0);
        assertEq(USDC.balanceOf(dada), 134 * USDC_ONE);
    }
    ...
    function test_supply_of_caca() public view {
        assertEq(sAVAX.balanceOf(caca), 0);
        assertEq(sUSDC.balanceOf(caca), 0);
        assertEq(sAVAX.totalOf(caca), 0);
        assertEq(sUSDC.totalOf(caca), 0);
    }

    function test_supply_of_dada() public view {
        assertEq(sAVAX.balanceOf(dada), 100 * AVAX_ONE);
        assertEq(sUSDC.balanceOf(dada), 0);
        assertEq(sAVAX.totalOf(dada), 100 * AVAX_ONE);
        assertEq(sUSDC.totalOf(dada), 0);
    }
    ...
    function test_borrow_of_caca() public view {
        assertEq(bAVAX.balanceOf(caca), 0);
        assertEq(bUSDC.balanceOf(caca), 0);
        assertEq(bAVAX.totalOf(caca), 0);
        assertEq(bUSDC.totalOf(caca), 0);
    }
    ...
    function test_lock_of_caca() public view {
        assertEq(pool.supplyLockOf(caca, AVAX), 0);
        assertEq(pool.supplyLockOf(caca, USDC), 0);
        assertEq(pool.borrowLockOf(caca, AVAX), 0);
        assertEq(pool.borrowLockOf(caca, USDC), 0);
    }

    function test_lock_of_dada() public view {
        assertEq(pool.supplyLockOf(dada, AVAX), 100 * AVAX_ONE);
        assertEq(pool.supplyLockOf(dada, USDC), 0);
        assertEq(pool.borrowLockOf(dada, AVAX), 0);
        assertEq(pool.borrowLockOf(dada, USDC), 0);
    }
    ...
    function test_health_of_dada() public view {
        uint256 supply = 8500.000000_000000_000000e18;
        Health memory health = pool.healthOf(dada);
...
contract PoolLiquidate_Event is PoolLiquidate {
    function setUp() public override {
        super.setUp();
        ///
        vm.startPrank(papa);
    ...
    function test_liquidate() public {
        vm.startPrank(papa);
        vm.expectEmit();
        emit Liquidate(papa, caca, 0);
        pool.liquidate(caca, 0);
        vm.stopPrank();
    }

    event Liquidate(address indexed, address indexed, uint8);
}

contract PoolLiquidate_Unauthorized is PoolLiquidate {
    function setUp() public override {
        super.setUp();
        ///
        vm.startPrank(papa);
        ...
        vm.stopPrank();
        ///
        acma.revokeRole(acma.POOL_SQUARE_ROLE(), address(pool));
    }
...
contract PoolLiquidate_SufficientHealth is TestBase {
    function setUp() public override {
        super.setUp();
        ///
        AVAX.transfer(caca, 200 * AVAX_ONE);
    ...
    function test_liquidate() public {
        ...
        vm.expectRevert(
            abi.encodeWithSelector(
                SUFFICIENT_HEALTH,
                caca,
                8500.000000_000000_000000e18,
                8415.000000_000000_000000e18
            )
        );
        pool.liquidate(caca, 0);
        vm.stopPrank();
    ...
    function test_health_of_caca() public view {
        Health memory health = pool.healthOf(caca);
        assertEq(health.wnav_supply, 8_500 * AVAX_ONE);
        assertEq(health.wnav_borrow, 8_415 * AVAX_ONE);
    }
    ...
    bytes4 SUFFICIENT_HEALTH = IPool.SufficientHealth.selector;
}
