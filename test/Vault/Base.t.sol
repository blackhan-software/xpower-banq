...
contract BaseTest is Test, IPoolRO {
    ...
    Token internal immutable token = new Token(100 * ONE);
    Token internal immutable buddy = new Token(100 * ONE);
    SupplyPosition internal supply;
    BorrowPosition internal borrow;
    ...
    function reset(VaultFee memory fee, IRModel memory irm) internal {
        acma = new Acma(address(this));
        supply = new SupplyPosition(this, token, buddy, irm, acma);
        borrow = new BorrowPosition(this, token, buddy, irm, acma);
        vault = new Vault(address(this), token, fee, supply, borrow, acma);
    }
    ...
    function healthOf(address) external pure override returns (Health memory) {
        return Health({wnav_supply: 1, wnav_borrow: 0});
    }
    ...
    function weightOf(IERC20) external pure override returns (Weight memory) {
        return Weight({supply: 170, borrow: 255});
    }
